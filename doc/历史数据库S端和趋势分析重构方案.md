...
查询10分钟3.58G数据，16点查询耗时58S左右

### 6.2. 设计二

#### 2024-06-24/point1

参考原历史数据库设计，每天创建一个目录，目录名以收集时间`yyyy-MM-dd`命名，每个点再生成一个库，库名以点名命名，库中只生成一张表，表名[PointValue]，定义如下：

| 字段名     | 数据类型 |                |
| ---------- | -------- | -------------- |
| Value      | double   | 点值           |
| Status     | TINYINT  | 告警状态       |
| UpdateTime | int      | 数据包头部定义 |

表设计不添加Id字段，未给UpdateTime字段添加索引，

**空间占用分析**：20万数据，500ms推送间隔

一条数据大小：8+1+4=13字节

一天：13 * 200000 * 2 * 60 * 60 * 24=418.4G

实际DB文件测试：一点一天生成2664KB，20万共2664*200000=508G

非当天数据，使用7z压缩，442*200000=84.3G

![](imgs/7z.png)

查询：一天16点建立16个数据库连接，查询16点所有数据，耗时2s以内左右

![](imgs/db2.png)

2天，32个数据库连接，查询16点所有数据，耗时4s以内，

![](imgs/db22.png)

3天，48个数据库连接，查询16点所有数据，耗时6s以内，
